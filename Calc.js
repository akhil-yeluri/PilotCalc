...
function SetAge()
{
    ...
    if(result==-1)
    {
        document.getElementById('ageText').innerHTML  = "DOB can't be greater than current date"
        return;
    }
  ...
  document.getElementById('ageText').innerHTML  = 'Your Age is '+Age.year+' Years '+Age.month+' Months '+Age.date+' Days';
}
...
function CalculateAge(DOB,Age)
{
    ...
    if(currentMonth == DOB.month && currentDate==DOB.date && currentYear==DOB.year)
        return 0;

    if((currentMonth <= DOB.month && currentDate<DOB.date && currentYear<=DOB.year) || currentYear<DOB.year)    
            return -1;
